...
from .transformer_blocks import Mlp, DropPath, trunc_normal_, nlc_to_nchw
...
class MultiScaleAttention(nn.Module):
    ...
    def forward(self, x, hw_shape):
        ...
        if self.pool_first:
            q_N = np.prod(q_shape) + 1 if self.has_cls_embed else np.prod(
                q_shape)
            k_N = np.prod(k_shape) + 1 if self.has_cls_embed else np.prod(
                k_shape)
            v_N = np.prod(v_shape) + 1 if self.has_cls_embed else np.prod(
                v_shape)

            q = q.permute(0, 2, 1, 3).reshape(B, q_N, -1)
            q = self.q(q).reshape(B, q_N, self.num_heads,
                                  -1).permute(0, 2, 1, 3)

            v = v.permute(0, 2, 1, 3).reshape(B, v_N, -1)
            v = self.v(v).reshape(B, v_N, self.num_heads,
                                  -1).permute(0, 2, 1, 3)

            k = k.permute(0, 2, 1, 3).reshape(B, k_N, -1)
            k = self.k(k).reshape(B, k_N, self.num_heads,
                                  -1).permute(0, 2, 1, 3)
...
class MViT(nn.Module):
    ...
    def __init__(self,
        ...
        dpr = [x.item() for x in torch.linspace(0, droppath_rate, depth)
               ]  # stochastic depth decay rule

        self.out_indices = [i[0] for i in dim_mul_cfg]

        # MViT backbone configs
        dim_mul, head_mul = torch.ones(depth + 1), torch.ones(depth + 1)
        for i in range(len(dim_mul_cfg)):
            dim_mul[dim_mul_cfg[i][0]] = dim_mul_cfg[i][1]
        for i in range(len(head_mul_cfg)):
            head_mul[head_mul_cfg[i][0]] = head_mul_cfg[i][1]
...
def mvitv2_tiny():
    model = MViT(depth=10,
                 droppath_rate=0.1,
                 dim_mul_cfg=[[1, 2.0], [3, 2.0], [8, 2.0]],
                 head_mul_cfg=[[1, 2.0], [3, 2.0], [8, 2.0]],
...
def mvitv2_small():
    model = MViT(depth=16,
                 droppath_rate=0.1,
                 dim_mul_cfg=[[1, 2.0], [3, 2.0], [14, 2.0]],
                 head_mul_cfg=[[1, 2.0], [3, 2.0], [14, 2.0]],
                 pool_q_stride=[[0, 1, 1], [1, 2, 2], [2, 1, 1], [3, 2, 2],
                                [4, 1, 1], [5, 1, 1], [6, 1, 1], [7, 1, 1],
                                [8, 1, 1], [9, 1, 1], [10, 1, 1], [11, 1, 1],
                                [12, 1, 1], [13, 1, 1], [14, 2, 2], [15, 1, 1]])
    return model
...
def mvitv2_base():
    model = MViT(depth=24,
                 droppath_rate=0.3,
                 dim_mul_cfg=[[2, 2.0], [5, 2.0], [21, 2.0]],
                 head_mul_cfg=[[2, 2.0], [5, 2.0], [21, 2.0]],
                 pool_q_stride=[[0, 1, 1], [1, 1, 1], [2, 2, 2], [3, 1, 1],
                                [4, 1, 1], [5, 2, 2], [6, 1, 1], [7, 1, 1],
                                [8, 1, 1], [9, 1, 1], [10, 1, 1], [11, 1, 1],
                                [12, 1, 1], [13, 1, 1], [14, 1, 1], [15, 1, 1],
                                [16, 1, 1], [17, 1, 1], [18, 1, 1], [19, 1, 1],
                                [20, 1, 1], [21, 2, 2], [22, 1, 1], [23, 1, 1]])
    return model
