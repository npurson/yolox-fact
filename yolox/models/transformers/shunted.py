...
from .transformer_blocks import DropPath, to_2tuple, trunc_normal_
import math
...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.sr_ratio > 1:
            ...
            v1 = v1 + self.local_conv1(
                v1.transpose(1, 2).reshape(B, -1, C // 2).transpose(1, 2).view(
                    B, C // 2, H // self.sr_ratio, W // self.sr_ratio)).view(
                        B, C // 2, -1).view(B, self.num_heads // 2,
                                            C // self.num_heads, -1).transpose(
                                                -1, -2)
            ...
            v2 = v2 + self.local_conv2(
                v2.transpose(1, 2).reshape(B, -1, C // 2).transpose(1, 2).view(
                    B, C // 2, H * 2 // self.sr_ratio,
                    W * 2 // self.sr_ratio)).view(B, C // 2, -1).view(
                        B, self.num_heads // 2, C // self.num_heads,
                        -1).transpose(-1, -2)
...
def shunted_t(pretrained=False, **kwargs):
    model = ShuntedTransformer(patch_size=4,
                               embed_dims=[64, 128, 256, 512],
                               num_heads=[2, 4, 8, 16],
                               mlp_ratios=[8, 8, 4, 4],
                               qkv_bias=True,
